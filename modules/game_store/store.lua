...
function init()
	...
	shopButton = modules.client_topmenu.addRightGameToggleButton('shopButton', tr('Shop'), '/images/topbuttons/shop',
		toggle, false, 8)
end
...
function onStoreCategories(categories)
	...
	for i, category in ipairs(categories) do
		...
		if category.icon and category.icon:len() > 0 then
			if localImageMode then
				cat:setIcon("images/32/" .. category.icon)
			else
...
function onCoinBalance(coins, transferableCoins)
	...
	if coinBalance then
		coinBalance:setText(formatNumbers(points))
	end
end
...
function transferAccept()
	...
	transferPointsWindow:getChildById('buttonOk').onClick = function(widget)
		...
		acceptWindow = displayGeneralBox(tr('Gift Tibia Coins'),
			tr("Do you want to transfer " .. transferableAmountSet .. " to " .. nickname:getText() .. "?"),
			{
...
function sendMessageBox(title, message, specialCallback)
	local okFunc = function()
		messageBox:destroy()
		messageBox = nil
		if specialCallback then openStoreWindow() end
	end
	messageBox = displayGeneralBox(title, message, { { text = tr('Ok'), callback = okFunc }, anchor =
	AnchorHorizontalCenter }, okFunc)
end
